...
from .simplecrop import SimpleCrop
...
class BmiSimpleCrop(Bmi):
    ...
    name = "simplecrop"
    exchange_item = {
        "solar_radiation": BmiVar(units="MJ / m2", dtype=float, size=1, intent="out"),
        "temperature_max": BmiVar(units="degC", dtype=float, size=1, intent="out"),
        "temperature_min": BmiVar(units="degC", dtype=float, size=1, intent="out"),
        "rain": BmiVar(units="mm", dtype=float, size=1, intent="out"),
        "irrigation": BmiVar(units="mm", dtype=float, size=1, intent="out"),
        "runoff": BmiVar(units="mm", dtype=float, size=1, intent="out"),
        "infiltration": BmiVar(units="mm", dtype=float, size=1, intent="out"),
        "drain": BmiVar(units="mm", dtype=float, size=1, intent="out"),
        "evapotranspiration": BmiVar(units="mm", dtype=float, size=1, intent="out"),
    ...
    def get_value(self, name: str, dest: numpy.ndarray) -> numpy.ndarray:
        ...
        """
        value_at_day = scipy.interpolate.interp1d(
            self._simplecrop._output["soil"]["day_of_year"],
            self._simplecrop._output["soil"][name],
        )
        dest[0] = value_at_day(self._time)
        return dest
    ...
    def get_var_itemsize(self, name: str) -> int:
        ...
        int
            Item size in bytes.
        """
        return numpy.dtype(self.exchange_item[name].dtype).itemsize
    ...
    def initialize(self, config_file: str) -> None:
        ...
        """
        self._simplecrop = SimpleCrop.from_config_file(config_file)
